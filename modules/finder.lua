...
AddPrefabPostInitAny(function(inst)
  if IsEndableMods(mod_ids) or inst:HasOneOfTags(tags_ignore) then return end
...
AddModRPCHandler('AWEI_SERVER_MOD', 'FINDER', function(player, prefab)
  ...
  for _, inst in pairs(ents) do
    local str = inst.net_awei_finder and inst.net_awei_finder:value() or ''
...
AddPrefabPostInit('inventory_classified', function(inst)
  if IsEndableMods(mod_ids) then return end
